...
contract ValinityReserveTreasury is UUPSUpgradeable, AccessControl, ReentrancyGuardTransient, Initializable, IERC721Receiver {
    ...
    /**
     * @notice Apply accrued interest by sending VY from collateral to a specified recipient
     * @dev Used by VLO to extract VY from a loan's locked collateral without an incoming
     *      asset deposit. The recipient is decided by VLO's routing policy:
     *        - Underwater loans (increaseLoan / liquidateUnderwater): recipient is VYT
     *          (no asset backing arriving; routing through buyback would force a second
     *          reserve withdrawal and dilute other users).
     *        - Routine accrual extraction (if VLO ever uses it for healthy loans):
     *          recipient could be any approved address (e.g. VBBO).
     *      Deducts vyAmount from the asset's collateralized VY and transfers it to the
     *      recipient. Caller-controlled recipient is intentional and gated by OFFICER_ROLE.
     * @param asset The reserve asset whose collateral pool to debit
     * @param vyAmount Amount of VY interest to apply
     * @param recipient Address to receive the VY (chosen by VLO; typically VYT)
     */
