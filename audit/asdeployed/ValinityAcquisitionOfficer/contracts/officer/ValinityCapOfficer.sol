    ...
    Initializable
{
    ...
    /// @dev Reserved storage for V2 upgrades. Kept at original size to
    ///      preserve storage layout; V3+ state is appended AFTER this gap.
    uint256[48] private __gapV2;

    // ── V3 storage (appended after __gapV2): cached top-LTV-F asset ──
    /// @notice Cached "highest LTV-F" asset used by addToHighestLTVFCap as a
    ///         hot-path hint. The cache is only a HINT — total VY accounting
    ///         is unaffected because every call still increments some asset's
    ///         cap by the full `amount`. Stale only as to which asset.
    /// @dev Packs with cacheCallsRemaining + cacheRefreshInterval (20+8+8=36B → 2 slots).
    address public cachedHighestLtvFAsset;
    /// @notice Calls remaining before a forced full-scan refresh. Decrements
    ///         each use; when 0 the next call does a full scan and resets to
    ///         `cacheRefreshInterval`. Also reset to 0 on addAsset/removeAsset.
    uint64 public cacheCallsRemaining;
    /// @notice Number of cache-hit calls between mandatory full scans.
    ///         0 disables the cache (always full-scan). Initialized to 20 via
    ///         migrateToCachedHighestLtvF().
    uint64 public cacheRefreshInterval;

    /// @dev Reserved storage for V3+ future upgrades.
    ...
    event HighestLtvFCacheRefreshed(address indexed asset, uint64 interval);
    event CacheRefreshIntervalUpdated(uint64 newInterval);
    ...
    function addAsset(
        ...
        // Asset set changed — force next addToHighestLTVFCap to rescan.
        cacheCallsRemaining = 0;
    ...
    function removeAsset(address asset) external onlyRole(ADMIN_ROLE) {
        ...
        // Asset set changed — force next addToHighestLTVFCap to rescan.
        if (cachedHighestLtvFAsset == asset) cachedHighestLtvFAsset = address(0);
        cacheCallsRemaining = 0;
    ...
    /**
     * @notice One-shot migration to seed the cached-highest-LTV-F state on V3 upgrade.
     * @dev Idempotent via reinitializer(3). Sets refresh interval to 20 calls.
     *      Cache itself is left empty so the next addToHighestLTVFCap does a full scan.
     */
    function migrateToCachedHighestLtvF()
        external
        onlyRole(ADMIN_ROLE)
        reinitializer(3)
    {
        cacheRefreshInterval = 20;
        cacheCallsRemaining = 0;
        cachedHighestLtvFAsset = address(0);
        emit CacheRefreshIntervalUpdated(20);
    }

    /**
     * @notice Set the cache refresh interval. 0 disables the cache (always full-scan).
     */
    function setCacheRefreshInterval(uint64 newInterval)
        external
        onlyRole(ADMIN_ROLE)
    {
        if (newInterval == cacheRefreshInterval) return;
        cacheRefreshInterval = newInterval;
        // Reset the counter so the new interval takes effect on next call.
        cacheCallsRemaining = 0;
        emit CacheRefreshIntervalUpdated(newInterval);
    }

    /**
     * @notice Force a refresh of the cached highest-LTV-F asset on the next call.
     * @dev Anyone may call this; it does not modify cap state, only the cache
     *      counter. Useful after a large reserve movement to make the next
     *      yield-payment route to the freshly-correct asset.
     */
    function invalidateHighestLtvFCache() external {
        cacheCallsRemaining = 0;
    }
     ...
     * @dev Called by yield-paying officers (e.g. VGO during keeper-reward swaps)
     *      so every freshly-minted VY is accounted for by raising backing on the
     *      asset that needs it most. LTV-F = (reserve USD value) / (VY cap),
     *      priced via VAO TWAP.
     *
     *      GAS PATH (V3): full-scan only every `cacheRefreshInterval` calls;
     *      the in-between calls reuse `cachedHighestLtvFAsset` (1 TWAP read
     *      instead of N). Accounting is unaffected — every call still adds the
     *      full `amount` to *some* asset's cap.
    ...
    function addToHighestLTVFCap(
        ...
        address highestAsset = cachedHighestLtvFAsset;
        uint64 remaining = cacheCallsRemaining;
        uint64 interval = cacheRefreshInterval;

        // ── Decide: cache hit or full scan ──
        bool useCache = (
            interval != 0 &&
            remaining != 0 &&
            highestAsset != address(0) &&
            _supported[highestAsset] &&
            _calculateLTVF(highestAsset, vrtAddress) != 0
        );

        if (useCache) {
            unchecked { cacheCallsRemaining = remaining - 1; }
        } else {
            // ── Full scan ──
            ...
            if (found == address(0)) revert NoValidAssetFound();

            highestAsset = found;
            cachedHighestLtvFAsset = found;
            // If interval is 0 the cache is disabled — keep counter at 0.
            cacheCallsRemaining = interval == 0 ? 0 : interval;
            emit HighestLtvFCacheRefreshed(found, interval);
        }

        // ── Apply the cap increase ──
